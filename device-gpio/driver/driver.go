...
func (s *Driver) HandleReadCommands(deviceName string, protocols map[string]contract.ProtocolProperties, reqs []dsModels.CommandRequest) (res []*dsModels.CommandValue, err error) {
	...
	for _, req := range reqs {
		s.lc.Debug(fmt.Sprintf("resource : %v ",req.DeviceResourceName))
		switch req.DeviceResourceName {
		case "humidity":
			{
				rxbuf, err :=s.gpiodevice.GetHumidity()
				if err != nil {
					return nil, err
				}
				cv ,_:= dsModels.NewInt32Value(req.DeviceResourceName,now,rxbuf)
				res = append(res, cv)
			}
		case "temperature":
			{
				rxbuf, err := s.gpiodevice.GetTemperature()
				if err != nil{
					return nil, err
				}
				cv, _:= dsModels.NewInt32Value(req.DeviceResourceName,now,rxbuf)
		...
		case "direction":
			{
				...
				cv := dsModels.NewStringValue(req.DeviceResourceName, now, rxbuf)
				res = append(res, cv)
			}
		case "value":
			{
